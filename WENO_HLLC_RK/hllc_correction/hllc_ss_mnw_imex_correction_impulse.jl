...
function rhs_hyperbolic(qn, epsilon, period, t, g, Imax, h, q, hn)
    ...
    for k = 1:Imax # at flux k
        ...
        hstar[k] =
            1 / 2 * (UL[1, k] + UR[1, k]) -
            1 / 4 * ((UR[2, k]/UR[1, k]) - (UL[2, k]/UL[1, k])) * (UL[1, k] + UR[1, k]) / (aleft[k] + aright[k])
        # calc. the estimated wave speed SL, SR, S*
        # left value
        if hstar[k] > UL[1, k]
            qqleft[k] = sqrt(1 / 2 * ((hstar[k] + UL[1, k]) * hstar[k]) / (UL[1, k]^2))
        else
            qqleft[k] = 1.0
        end
        SL[k] = (UL[2, k] / UL[1, k]) - aleft[k] * qqleft[k]
        # right value
        if hstar[k] > UR[1, k]
            qqright[k] = sqrt(1 / 2 * ((hstar[k] + UR[1, k]) * hstar[k]) / (UR[1, k]^2))
        else
            qqright[k] = 1.0
        end
        SR[k] = (UR[2, k] / UR[1, k]) + aright[k] * qqright[k]
        Sstar[k] =
            (SL[k] * UR[1, k] * ((UR[2, k]/UR[1, k]) - SR[k]) - SR[k] * UL[1, k] * ((UL[2, k]/UL[1, k]) - SL[k])) /
            (UR[1, k] * ((UR[2, k]/UR[1, k]) - SR[k]) - UL[1, k] * ((UL[2, k]/UL[1, k]) - SL[k]))
        # calc. approx states for U*L, U*R
        ULstar[:, k] = UL[1, k] * ((SL[k] - (UL[2, k]/UL[1, k])) / (SL[k] - Sstar[k])) .* [1.0; Sstar[k]]
        URstar[:, k] = UR[1, k] * ((SR[k] - (UR[2, k]/UR[1, k])) / (SR[k] - Sstar[k])) .* [1.0; Sstar[k]]
...
const hn = 0.0053339999999999
const un = 1.2809997245901343 # U=Frn*sqrt(gH)
const qn = hn * un
const g = 9.810
const sine_theta = 0.1192
const tan_theta = sine_theta / sqrt(1 - sine_theta^2)
const cf = g * tan_theta * 2 * hn / (un^2)
const epsilon = 0.20
const period = 1.015273625695317
...
const txt_output = 10
const graphic_output = 0.50
const peak_output = 0.5
...
for i = 1:nt
    ...
    if mod(t, graphic_output) < dt
        ...
        KE = 1.0 / 2.0 * (h .* u.^2)
        Fr = u ./ sqrt.(g * h)
        CSV.write("./WSP1/" * @sprintf("%.1f", t) * ".csv", DataFrame([x h[1:Imax] u[1:Imax] KE[1:Imax] Fr[1:Imax]]))
    end
