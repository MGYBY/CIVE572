...
function rhs_hyperbolic(qn, epsilon, period, t, g, Imax, h, q)
...
for i = 1:nt
    ...
    Fhllc .= rhs_hyperbolic(qn, epsilon, period, t, g, Imax, h, q)
    # solve the conserved form and obtain h and q (namely U)
    # hyperbolic problem
    # RK4
    for k = 2:Imax
        hmed1[k] = h[k] - 1.0 / 2.0 * dt / dx * (Fhllc[1, k] - Fhllc[1, k - 1])
        qmed1[k] = q[k] - 1.0 / 2.0 * dt / dx * (Fhllc[2, k] - Fhllc[2, k - 1])
    end

    Fhllc .= rhs_hyperbolic(qn, epsilon, period, t, g, Imax, hmed1, qmed1)
    for k = 2:Imax
        hmed2[k] = h[k] - 1.0 / 2.0 * dt / dx * (Fhllc[1, k] - Fhllc[1, k - 1])
        qmed2[k] = q[k] - 1.0 / 2.0 * dt / dx * (Fhllc[2, k] - Fhllc[2, k - 1])
    end

    Fhllc .= rhs_hyperbolic(qn, epsilon, period, t, g, Imax, hmed2, qmed2)
    for k = 2:Imax
        hmed3[k] = h[k] - 1.0 * dt / dx * (Fhllc[1, k] - Fhllc[1, k - 1])
        qmed3[k] = q[k] - 1.0 * dt / dx * (Fhllc[2, k] - Fhllc[2, k - 1])
    end
    Fhllc .= rhs_hyperbolic(qn, epsilon, period, t, g, Imax, hmed3, qmed3)
    for k = 2:Imax
        h[k] = 1.0 / 3.0 * (-h[k] + hmed1[k] + 2 * hmed2[k] + hmed3[k]) - 1 / 6 * (dt / dx * (Fhllc[1, k] - Fhllc[1, k - 1]))
        q[k] = 1.0 / 3.0 * (-q[k] + qmed1[k] + 2 * qmed2[k] + qmed3[k]) - 1 / 6 * (dt / dx * (Fhllc[2, k] - Fhllc[2, k - 1]))
    end

    # solve the ODE for time integration (RK3 or RK4)
    for k = 2:Imax
        qmed1[k] = q[k] + 1.0 / 2.0 * dt * (HLLC_source_term(h[k], q[k], tan_theta, cf))
    end
    for k = 2:Imax
        qmed2[k] = q[k] + 1.0 / 2.0 * dt * (HLLC_source_term(h[k], qmed1[k], tan_theta, cf))
    end
    for k = 2:Imax
        qmed3[k] = q[k] + 1.0 * dt * (HLLC_source_term(h[k], qmed2[k], tan_theta, cf))
    end
    for k = 2:Imax
        q[k] = 1.0 / 3.0 * (-q[k] + qmed1[k] + 2 * qmed2[k] + qmed3[k]) + 1 / 6 * (dt * (HLLC_source_term(h[k], qmed3[k], tan_theta, cf)))
    end
