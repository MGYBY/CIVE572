...
function rhs_hyperbolic(qn, epsilon, period, t, g, Imax, h, q, hn)
    ...
    # instant dist.
    if t < period
        h[1] = hn * (1 + epsilon * (sin(2 * pi ./ period * t))) 
        q[1] = Fr * sqrt(g * h[1]^3)
    else
        h[1] = hn
        q[1] = qn
...
for i = 1:nt
    ...
    if mod(t, graphic_output) < dt
        ...
        png("./WSP1/" * @sprintf("%.1f", t) * ".png")
        u = q ./ h
        KE = 1.0 / 2.0 * (h .* u.^2)
        Fr = u ./ sqrt.(g * h)
        CSV.write("./WSP1/" * @sprintf("%.1f", t) * ".csv", DataFrame([x h[1:Imax] u[1:Imax] KE[1:Imax] Fr[1:Imax]], :auto))
    end
